...
int ans[N], sq[N], invsq[N], getln[N], exp0[N], f[N] = {1,6,1}, g[N] = {1,1};
//sq = \sqrt(f)
//invsq = sq^{-1}
//getln = ln(g+sq)
//exp0 = e^(getln1*(x+1))
...
signed main(){
    fastio
    int n = 1<<15, x, y, inv2 = fastpow(2,MOD-2);
    cin >> x >> y;
    NTT.sqrt(f,sq,n);
    for(int i = 0;i < n;i++) g[i] = (g[i]+sq[i])%MOD*inv2%MOD;
    NTT.ln(g,getln,n);
	for(int i = 0;i < n;i++) getln[i] = getln[i]*(x+1)%MOD;
    NTT.exp(getln,exp0,n);

    NTT.inverse(sq,invsq,n);

    NTT.dft(exp0), NTT.dft(invsq);
    for(int i = 0;i < n<<1;i++) ans[i] = exp0[i]*invsq[i]%MOD;
    NTT.idft(ans);
	for(int i = 1;i <= y;i++) cout << (i <= x ? ans[i] : 0) << " ";
}